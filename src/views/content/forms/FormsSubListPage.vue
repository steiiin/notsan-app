...
const list = computed(() => content.findFormListById(props.id))

const entries = computed(() => {
  if (!list.value) { return [] }
  return list.value.entries.map(entry => ({
    ...entry,
    path: `/tabs/forms/list/${list.value?.id}/${entry.id}`
  }))
})
...
router.afterEach(async (to, from) => {
  ...
  if (to.fullPath.includes('forms/list') && from.fullPath.includes('forms/list')) {
