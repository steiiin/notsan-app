<template>
  ...
  <ns-flow-action ref="action_simple">
    <h2>Allergische Reaktion (I)</h2>
    <p>Leichte Auswirkungen.</p>
    <hr>
    <ul head>
      <li>Hautreaktionen: <i>Juckreiz, Quaddeln, Rötung</i></li>
      <li>Abdominelle Beschwerden: <i>Schmerzen, Übelkeit</i></li>
    </ul>
  </ns-flow-action>
  <ns-flow-action ref="action_severe">
    ...
    <hr>
    <ul head>
      <li>A: <i>Schwellung, Heiserkeit, Stridor</i></li>
      <li>B: <i>Tachypnoe, Obstruktion</i></li>
      <li>C: <i>Kaltschweißigkeit, Schockig</i></li>
      <li>D: <i>Verwirrung, Unruhe, Eintrübung</i></li>
    </ul>
  </ns-flow-action>
...
const action_simple = ref<InstanceType<typeof NsFlowAction> | null>(null)
const action_severe = ref<InstanceType<typeof NsFlowAction> | null>(null)

function handleAction(payload: FlowActionPayload) {
  if (payload.key === 'simple') {
    action_simple.value?.presentPopover(payload)
  }
